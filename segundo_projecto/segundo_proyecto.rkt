        ...
        [mayor (car fil-col)])
          (for ([i (in-range tam)])
                (when (> (list-ref fil-col i) mayor ); aqui retorna void cuando no se cumple
                     ((set! vistos (add1 vistos)) (set! mayor (list-ref fil-col i))))
...
(define (resolve grilla tam fil col vistas)
  (cond
    [(= fil tam) (display(for*/list ([i (in-range tam)])
                          (vector->list (list-ref grilla i))))]
    [(= col (sub1 tam)) (resolve grilla tam (add1 fil) 0 vistas)]
    [else ( let*([posibles (vector-ref (list-ref grilla fil)col)]
                 [result #f]
                 [valor 0])
             (for* ([ i (in-range (length posibles))])
               (if ( equal? result #f)
                   ((set! valor (list-ref posibles i))
                    (when (elegible grilla fil col tam vistas valor)
